...
function HasPermission(src, permissions)
    if CoreName == "qb-core" or CoreName == "qbx_core" then
...
RegisterNetEvent('nvrs-vehicleshop:server:triggerCallback', function(name, ...)
    local src = source
    TriggerCallback(name, src, function(...)
        TriggerClientEvent('nvrs-vehicleshop:client:triggerCallback', src, name, ...)
    end, ...)
end)