...
window.addEventListener('message', function(event) {
    ...
    if (ed.action === "openDealership") {
		...
		ed.vehicles.forEach(function(vehicleData, index) {
			...
			// 	clFunc('selectSpotVehicle', vehicleData.model)
			// }
		});
		...
		ed.vehicles.forEach(function(vehicleData, index) {
			if (vehicleData.stock >= 1) {
				...
				<div class="showroomDivBottomRightDiv" id="showroomDivBottomRightDiv-${vehicleData.name}">
					...
					<div class="showroomDivBottomRightDivBottom" id="showroomDivBottomRightDivBottom-${vehicleData.model}" onclick="clFunc('selectSellVehicle', '${vehicleData.model}', '${vehicleData.price}')" style="gap: 0.2vw;"><span>Send Request</span><div id="showroomDivBottomRightDivBottomChevrons"><i class="fal fa-chevron-right"></i><i class="fal fa-chevron-right"></i><i class="fal fa-chevron-right"></i></div></div>
				</div>`;
...
function clFunc(name1, name2, name3, name4, name5, name6) {
	if (name1 === "showCategoryVehicles") {
			...
			if (vehData.category === name2) {
				...
				<div class="MDBCDVehicleDiv" id="MDBCDVehicleDiv-${vehData.spawncode}" onclick="clFunc('chooseVeh', '${vehData.spawncode}', '${vehData.categoryLabel}', '${vehData.price}', '${vLabel}', '${vehData.stock}')">
					...
					<div id="MDBCDVehicleDivBottom">
						<div class="MDBCDVehicleDivTopRightSide" style="bottom: 0; color: rgba(144, 255, 215, 0.77); text-shadow: 0px 0px 4.8px rgba(144, 255, 215, 0.55);"><span>$${vehData.price}</span></div>
					</div>
				</div>`;
		...
		document.getElementById("vehPrice").innerHTML="$" + formatter.format(Number(name4));
		currentPrice = "$" + formatter.format(Number(name4));
		...
		document.getElementById("confirmSpan").innerHTML=`Are you authorizing payment via ${name2} for the purchase of the ${currentVehLabel} model vehicle for ${currentPrice} dollars? Please confirm with 'Yes' or 'No'.`;
	...
	document.getElementById("mainDivBottomCenterDiv").innerHTML = vehicles.map((vehicle) => {
		...
		return (
			`<div class="MDBCDVehicleDiv ${divClass}" id="MDBCDVehicleDiv-${spawncode}" onclick="clFunc('chooseVeh', '${spawncode}', '${categoryLabel}', '${price}', '${vLabel}', '${stock}')">
				...
				<div id="MDBCDVehicleDivBottom">
					<div class="MDBCDVehicleDivTopRightSide" style="color: rgba(144, 255, 215, 0.77); text-shadow: 0px 0px 4.8px rgba(144, 255, 215, 0.55);"><span>$${price}</span></div>
				</div>
			</div>`
		)
	}).join('');
...
const displayShowroomVehicle2 = (vehicles) => {
	let div = "showroomDivBottomRight2";
	if (currentShowroomMenu === "showAllVehiclesToAddStock") {
		div = "showroomDivBottomRight3";
	}
	document.getElementById(div).innerHTML = vehicles.map((vehicle) => {
		...
		return (
			`<div class="showroomDivBottomRightDiv" id="showroomDivBottomRightDiv-${name}">
				...
				<div class="showroomDivBottomRightDivBottom" id="showroomDivBottomRightDivBottom-${model}" onclick="clFunc('selectSellVehicle', '${model}', '${price}')" style="gap: 0.2vw;"><span>Send Request</span><div id="showroomDivBottomRightDivBottomChevrons"><i class="fal fa-chevron-right"></i><i class="fal fa-chevron-right"></i><i class="fal fa-chevron-right"></i></div></div>
			</div>`
		)
	}).join('');
}